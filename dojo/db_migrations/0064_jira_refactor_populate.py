...
class Migration(migrations.Migration):
    ...
    def move_jira_creation_changed(self, schema_editor):
        ...
        JIRA_Issue = self.get_model('dojo', 'JIRA_Issue')
    ...
    def populate_jira_project(self, schema_editor):
        # We can't import models directly as it may be a newer
        # version than this migration expects. We use the historical version.
        ...
        for jira_issue in JIRA_Issue.objects.all().select_related('jira_project').prefetch_related('finding__test__engagement__product'):
            # try:
            if not jira_issue.jira_project:
                if jira_issue.finding:
    ...
    def populate_jira_instance_name_if_empty(self, schema_editor):
        # We can't import models directly as it may be a newer
        # version than this migration expects. We use the historical version.
        logger.info('populating JIRA_Instance.configuration_name with url if empty')
        JIRA_Instance = self.get_model('dojo', 'JIRA_Instance')
    ...
    def show_info(self, schema_editor):
